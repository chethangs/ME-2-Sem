...
def cg(field,y,total_entropy):
	...
	for i in range(len(positive_list)):
		entropy = ce(positive_list[i],negative_list[i])
...
def calculate_total_entropy(Y):
	...
	for i in range(len(y)):
		if Y[i] == 1:
			...
			neg = neg + 1
	##print(pos,neg)
	total_entropy = ce(pos,neg)
	return total_entropy


data = [[1,1,1,2,0,2,1,0,0,2,0,1,1,2],
...
for i in range(len(data)):
	gain.append(cg(data[i],y,total_entropy))
print(gain)